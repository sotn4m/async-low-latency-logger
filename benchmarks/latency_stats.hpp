...
struct LatencyStats {
  ...
  static LatencyStats from_samples (
      std::vector<std::chrono::nanoseconds>& samples);

  void print (std::string_view label) const;
};

inline LatencyStats LatencyStats::from_samples (
    std::vector<std::chrono::nanoseconds>& samples) {
  LatencyStats stats {};
  if (samples.empty ()) {
    return stats;
...
inline void LatencyStats::print (std::string_view label) const {
  const auto us = [] (std::chrono::nanoseconds d) {
    return static_cast<double> (d.count ()) / 1000.0;
  };
  std::printf (
      "%-8s n=%-8zu p50=%9.2fus p90=%9.2fus p99=%9.2fus p99.9=%9.2fus "
      "p99.99=%9.2fus max=%9.2fus\n",
      label.data (), count, us (p50), us (p90), us (p99), us (p999), us (p9999),
      us (max));
}
